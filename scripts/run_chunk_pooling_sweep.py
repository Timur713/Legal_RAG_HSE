...
from legal_rag.retrieval import CHUNK_SCORE_AGGREGATIONS  # noqa: E402


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run strict-CV sweep over chunk score aggregation methods."
    )
    parser.add_argument(
        "--paths",
    ...
    parser.add_argument(
        ...
        help="Retriever family to evaluate with different chunk aggregation methods.",
    )
    parser.add_argument(
        "--chunk-score-aggregations",
        nargs="*",
        default=list(CHUNK_SCORE_AGGREGATIONS),
    ...
    parser.add_argument(
        ...
        type=int,
        default=800,
        help="Chunk step in characters for structure-aware chunked retrievers.",
    )
    parser.add_argument(
        "--top-k",
        type=int,
...
def build_run_command(args: argparse.Namespace, aggregation: str, experiment_name: str) -> list[str]:
    command = [
        sys.executable,
...
def main() -> int:
    ...
    for aggregation in args.chunk_score_aggregations:
        experiment_name = f"{args.experiment_prefix}_{args.retriever}_{aggregation}"
        command = build_run_command(args, aggregation, experiment_name)
        ...
        rows.append(
            {
                ...
                "chunk_score_aggregation": aggregation,
